...
import { AddressCases, AddressSubsections, Event, EventLocationAddressCases, ISerializedForm } from '../types/types'
...
import {
  ...
  birthAttendantNotNone,
  birthAttendantOnlyNurse
...
import { getCommonSectionMapping, getCustomFieldMapping } from '@countryconfig/utils/mapping/field-mapping-utils'
import { getBirthAttendantFullName, getBirthAttendantID, getReasonForLateRegistration } from '../custom-fields'
import { getIDNumberFields, getIDType } from '../custom-fields'
import { getAddressFields } from '../addresses/address-fields'
import { custom } from 'joi'
import { getAddressSubsection } from '@countryconfig/utils/address-utils'
...
const customFields = getAddressFields('child', AddressCases.PRIMARY_ADDRESS).map(field=>{
  const fieldId: string = `birth.child.child-view-group.${field.name}`
  return{
    ...field, 
      ...
      groups: [
        {
          ...
          fields: [
            ...
            ...getAddressSubsection(
              ...
              {
                id:'form.field.label.placeOfResidence',
                description:'a label for place of residence fields in child section',
                defaultMessage:"Place of residence"
            }),
            ...customFields,
            attendantAtBirth,
            getBirthAttendantFullName([
              {
                action: 'hide', 
                expression: birthAttendantNotNone
              }
            ]),
            ...
            getBirthAttendantID([
              {
                action: 'hide', 
                expression: birthAttendantOnlyNurse
              }
            ]),
            birthType,
            weightAtBirth
